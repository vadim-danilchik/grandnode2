...
{
    public class ShippingPickupPointRateProvider : IShippingRateCalculationProvider
    {
        #region Fields
        ...
        private readonly ShippingPickupPointRateSettings _ShippingPickupPointRateSettings;

        #endregion

        #region Ctor
        public ShippingPickupPointRateProvider(
            ...
            ShippingPickupPointRateSettings ShippingPickupPointRateSettings
            )
        {
            ...
            _ShippingPickupPointRateSettings = ShippingPickupPointRateSettings;
        }
        #endregion
        ...
        public async Task<GetShippingOptionResponse> GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            {
                Name = _translationService.GetResource("Shipping.ShippingPickupPoint.PluginName"),
                Description = _translationService.GetResource("Shipping.ShippingPickupPoint.PluginDescription"),
                Rate = 0,
                ShippingRateProviderSystemName = "Shipping.ShippingPickupPoint"
            });
        ...
        public async Task<IList<string>> ValidateShippingForm(IFormCollection form)
        {
            ...
            if (string.IsNullOrEmpty(shippingOptionId))
                return new List<string>() { _translationService.GetResource("Shipping.ShippingPickupPoint.SelectBeforeProceed") };

            if (shippingMethodName != _translationService.GetResource("Shipping.ShippingPickupPoint.PluginName"))
                throw new ArgumentException("shippingMethodName");

            var chosenShippingOption = await _ShippingPickupPointService.GetStoreShippingPickupPointById(shippingOptionId);
            ...
            string.Format("<strong>{0}:</strong> {1}<br><strong>{2}:</strong> {3}<br>",
                _translationService.GetResource("Shipping.ShippingPickupPoint.Fields.ShippingPickupPointName"), chosenShippingOption.ShippingPickupPointName,
                _translationService.GetResource("Shipping.ShippingPickupPoint.Fields.Description"), chosenShippingOption.Description
            );
            ...
            var serializedObject = new Domain.ShippingPickupPointSerializable()
            {
                Id = chosenShippingOption.Id,
                ShippingPickupPointName = chosenShippingOption.ShippingPickupPointName,
            ...
            {
                var xmlS = new XmlSerializer(typeof(Domain.ShippingPickupPointSerializable));
        ...
        {
            return await Task.FromResult("ShippingPickupPoint");
        }
        ...
        public string ConfigurationUrl => ShippingPickupPointRateDefaults.ConfigurationUrl;

        public string SystemName => ShippingPickupPointRateDefaults.ProviderSystemName;

        public string FriendlyName => _translationService.GetResource(ShippingPickupPointRateDefaults.FriendlyName);

        public int Priority => _ShippingPickupPointRateSettings.DisplayOrder;
