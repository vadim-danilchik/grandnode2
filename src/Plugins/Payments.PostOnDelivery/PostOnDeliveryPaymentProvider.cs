...
{
    public class PostOnDeliveryPaymentProvider : IPaymentProvider
    {
        private readonly ITranslationService _translationService;
        private readonly IServiceProvider _serviceProvider;

        private readonly PostOnDeliveryPaymentSettings _PostOnDeliveryPaymentSettings;

        public PostOnDeliveryPaymentProvider(
            ITranslationService translationService,
            IServiceProvider serviceProvider,
            PostOnDeliveryPaymentSettings PostOnDeliveryPaymentSettings
            )
        {
            _translationService = translationService;
            _serviceProvider = serviceProvider;
            _PostOnDeliveryPaymentSettings = PostOnDeliveryPaymentSettings;
        }

        public string ConfigurationUrl => PostOnDeliveryPaymentDefaults.ConfigurationUrl;

        public string SystemName => PostOnDeliveryPaymentDefaults.ProviderSystemName;

        public string FriendlyName => _translationService.GetResource(PostOnDeliveryPaymentDefaults.FriendlyName);

        public int Priority => _PostOnDeliveryPaymentSettings.DisplayOrder;
        ...
        {
            if (_PostOnDeliveryPaymentSettings.ShippableProductRequired && !cart.RequiresShipping())
                return true;
        ...
        {
            if (_PostOnDeliveryPaymentSettings.AdditionalFee <= 0)
                return _PostOnDeliveryPaymentSettings.AdditionalFee;

            double result;
            if (_PostOnDeliveryPaymentSettings.AdditionalFeePercentage)
            {
                //percentage
                ...
                result = (double)((((float)subtotal.subTotalWithDiscount) * ((float)_PostOnDeliveryPaymentSettings.AdditionalFee)) / 100f);
            }
            else
            {
                //fixed value
                result = _PostOnDeliveryPaymentSettings.AdditionalFee;
            }
        ...
        {
            viewComponentName = "PaymentPostOnDelivery";
        }
        ...
        {
            return await Task.FromResult(_PostOnDeliveryPaymentSettings.SkipPaymentInfo);
        }

        public async Task<string> Description()
        {
            return await Task.FromResult(_translationService.GetResource("Plugins.Payment.PostOnDelivery.PaymentMethodDescription"));
        }

        public string LogoURL => "/Plugins/Payments.PostOnDelivery/logo.jpg";
